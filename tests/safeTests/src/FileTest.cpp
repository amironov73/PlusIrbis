...
TEST_CASE("File_create_1", "[file]")
{
    ...
    {
        auto file = irbis::File::create (fileName);
...

    {
        ...
        auto file = irbis::File::openRead (fileName);
...
TEST_CASE("File_insistCreate_1", "[file]")
{
    const auto tempDirectory = irbis::IO::getTempDirectory();
    REQUIRE (irbis::IO::directoryExist (tempDirectory));
    auto fileName = irbis::IO::combinePath (tempDirectory, L"irbis.tmp");
    ...
    {
        auto file = irbis::File::insistCreate (fileName);
        file.write (original, irbis::size (original));
        file.close();
    }
...
TEST_CASE("File_insistCreate_2", "[file]")
{
    const auto tempDirectory = irbis::IO::getTempDirectoryNarrow();
    REQUIRE (irbis::IO::directoryExist (tempDirectory));
    auto fileName = irbis::IO::combinePath (tempDirectory, "irbis.tmp");
...
TEST_CASE("File_openRead_1", "[file]")
{
...
TEST_CASE("File_openRead_2", "[file]")
{
...
TEST_CASE("File_insistOpenRead_1", "[file]")
{
    auto path = whereTestData();
    REQUIRE (!path.empty());
    path = irbis::IO::combinePath (path, L"TEST1.ISO");
    irbis::IO::convertSlashes (path);
    irbis::Byte buffer[100];
    auto file = irbis::File::insistOpenRead (path);
    auto position = file.tell();
    CHECK (position == 0);
    auto read = file.read (buffer, irbis::size (buffer));
    CHECK (read == 100);
...
TEST_CASE("File_insistOpenRead_2", "[file]")
{
    auto path = irbis::toUtf (whereTestData());
    REQUIRE (!path.empty());
    path = irbis::IO::combinePath (path, "TEST1.ISO");
...
TEST_CASE("File_openWrite_1", "[file]")
{
    auto path = whereTestData();
    REQUIRE (!path.empty());
    path = irbis::IO::combinePath (path, L"TEST1.ISO");
    irbis::IO::convertSlashes (path);
    auto file = irbis::File::openWrite (path);
    auto position = file.tell();
    CHECK (position == 0);
    file.seek (86);
    position = file.tell();
...
TEST_CASE("File_openWrite_2", "[file]")
{
    auto path = irbis::toUtf (whereTestData());
    REQUIRE (!path.empty());
    path = irbis::IO::combinePath (path, "TEST1.ISO");
...
TEST_CASE("File_insistOpenWrite_1", "[file]")
{
    auto path = whereTestData();
    REQUIRE (!path.empty());
    path = irbis::IO::combinePath (path, L"TEST1.ISO");
    irbis::IO::convertSlashes (path);
    auto file = irbis::File::insistOpenWrite (path);
    auto position = file.tell();
    CHECK (position == 0);
    file.seek (86);
    position = file.tell();
...
TEST_CASE("File_insistOpenWrite_2", "[file]")
{
    auto path = irbis::toUtf (whereTestData());
    REQUIRE (!path.empty());
    path = irbis::IO::combinePath (path, "TEST1.ISO");
...
TEST_CASE("File_readChar_1", "[file]")
{
    auto path = whereTestData();
    REQUIRE (!path.empty());
    path = irbis::IO::combinePath (path, L"org.mnu");
...
TEST_CASE("File_readInt32_1", "[file]")
{
    auto path = whereTestData();
    REQUIRE (!path.empty());
...
TEST_CASE("File_readInt64_1", "[file]")
{
    auto path = whereTestData();
    REQUIRE (!path.empty());
    path = irbis::IO::combinePath (path, L"Irbis64/Datai/IBIS/ibis.xrf");
...
TEST_CASE("File_readNarrow_1", "[file]")
{
    auto path = whereTestData();
    REQUIRE (!path.empty());
    path = irbis::IO::combinePath (path, L"org.mnu");
...
TEST_CASE("File_writeInt32_1", "[file]")
{
...
TEST_CASE("File_writeInt64_1", "[file]")
{
    auto path = irbis::IO::getTempDirectory();
    REQUIRE (!path.empty());
    REQUIRE (irbis::IO::directoryExist (path));
