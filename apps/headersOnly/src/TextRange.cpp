...
struct TextRange final
{
    ...
    IRBIS_NODISCARD
    int compareTo (const TextRange &that) const noexcept
    {
    ...
    /// \brief Сравнивает поэлементно диапазон (начиная с текущей позиции) с указанной строкой.
    /// \param that C-строка для сравнения.
    /// \return 0, если равны, меньше 0, если this меньше, больше 0, если this больше.
    IRBIS_NODISCARD
    int compareTo (const CharType *that) const noexcept
    {
        return this->compareTo (that, that + CharTraits::length (that));
    }
    ...
    /// \return 0, если равны, меньше 0, если this меньше, больше 0, если this больше.
    IRBIS_NODISCARD
    int compareTo (const std::basic_string <CharType> &that) const noexcept
    {
        return this->compareTo (std::begin (that), std::end (that));
    }
